        ...
        <div className="p-8 space-y-8 max-w-3xl mx-auto animate-in fade-in duration-700">
            ...
            <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-5">
                <div className="flex items-center gap-2">
                    <User className="w-4 h-4 text-[#ffe14d]" />
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-neutral-600 mb-1">Connected since</p>
                        <p className="text-neutral-300">{formatDate(account?.connectedAt ?? null)}</p>
                    </div>
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-neutral-600 mb-1">Connection status</p>
                        <p className={expired ? "text-red-400" : "text-green-400"}>
                ...
                <button
                    ...
                    className="flex items-center gap-2 h-9 px-4 rounded-full border border-white/10 text-neutral-300 hover:text-white hover:border-white/30 font-mono-ui text-[11px] font-bold uppercase tracking-widest transition-colors"
                >
            ...
            <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-4">
                <div className="flex items-center gap-2">
                    <Webhook className="w-4 h-4 text-[#ffe14d]" />
                ...
                <div className="flex items-center gap-2">
                    <code className="flex-1 min-w-0 truncate bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-neutral-300">
                        {webhookUrl}
                    </code>
                    <button
                        onClick={handleCopyWebhookUrl}
                        className="shrink-0 w-9 h-9 flex items-center justify-center rounded-xl border border-white/10 text-neutral-400 hover:text-white hover:border-white/30 transition-colors"
                        title="Copy"
                    >
                ...
                <div className="flex items-center gap-2 text-xs">
                    <span
                        className={`w-1.5 h-1.5 rounded-full ${account?.webhookVerifyTokenConfigured ? "bg-green-400" : "bg-red-400"}`}
                    />
            ...
            <section className="rounded-2xl border border-red-500/20 bg-red-500/[0.03] p-6 space-y-4">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <button
                        onClick={logout}
                        className="flex items-center gap-2 h-9 px-4 rounded-full border border-white/10 text-neutral-300 hover:text-white hover:border-white/30 font-mono-ui text-[11px] font-bold uppercase tracking-widest transition-colors shrink-0"
                    >
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap pt-4 border-t border-white/5">
                    <div>
                        <p className="text-sm text-white">Disconnect Instagram</p>
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 h-9 px-4 rounded-full border border-red-500/30 bg-red-500/10 text-red-300 hover:bg-red-500/20 font-mono-ui text-[11px] font-bold uppercase tracking-widest transition-colors shrink-0 disabled:opacity-50"
                    >
